...
const emptyWorkspace = { discounts: [], products: [], operations: [], tasks: [], launches: [] };
const nav = [
  ['overview', '⌂', '运营总览'],
  ['discounts', '%', '商品折扣'],
  ['data', '⌁', '运营数据'],
...
const hasWorkspaceRecords = (data) => ['discounts', 'products', 'operations', 'tasks', 'launches'].some((key) => Array.isArray(data?.[key]) && data[key].length > 0);
...
export default function App() {
  ...
  const saveProduct = (event) => {
    ...
    const next = { id: editing?.id || uid(), category: data.get('category'), productName: String(data.get('productName')).trim(), specs, cost: specs[0]?.cost || 0, updatedAt: new Date().toISOString() };
  ...
  const saveDiscount = async (event) => {
    ...
    const profits = summary.specs.map((spec) => spec.salePrice * selectedDiscount - spec.cost - 3);
...
function Products({ records, search, setSearch, onEdit, onDelete }) {
  ...
  const filtered = records.filter((item) => {
    ...
    const matchesSearch = `${item.productName}${normalizeProductSpecs(item).map((spec) => spec.name).join('')}`.toLowerCase().includes(search.toLowerCase());
...
function Discounts({ records, search, setSearch, onEdit, onDelete }) {
  ...
  return <>
    ...
    <TableShell title="商品折扣记录" subtitle={`按（成本 + 6）÷ 售价计算最低可报档位${tierFilter ? ` · 当前查看可报 ${discountText(tierFilter)} 的商品` : ''}`} search={search} setSearch={setSearch}>
      <table><thead><tr><th>商品</th><th>店铺</th><th>成本</th><th>售价</th><th>最低折扣</th><th>最低可报</th><th>折后价</th><th>折后利润</th><th>状态</th><th>操作</th></tr></thead><tbody>
        {filtered.map((item) => { const specs = normalizeDiscountSpecs(item); const summary = summarizeDiscountSpecs(specs); const profits = specs.map((spec) => spec.salePrice * Number(item.selectedDiscount) - spec.cost - 3); return <tr key={item.id}><td><div className="product-cell"><span className="thumb">{item.imageDataUrl ? <img src={item.imageDataUrl} alt="" /> : '折'}</span><span><strong>{item.productName}</strong><small>{item.productCode}{specs.length > 1 ? ` · ${specs.length}个规格 · 限制规格：${summary.limitingSpec?.name}` : ` · ${specs[0]?.name}`}</small></span></div></td><td><Badge>{item.store}</Badge></td><td>{valueRange(specs.map((spec) => spec.cost), money)}</td><td>{valueRange(specs.map((spec) => spec.salePrice), money)}</td><td>{discountText(summary.minimumRatio)}</td><td><Badge>{discountText(summary.recommendedDiscount)}</Badge></td><td>{valueRange(specs.map((spec) => spec.salePrice * Number(item.selectedDiscount)), money)}</td><td className={Math.min(...profits) < 0 ? 'negative' : 'positive'}>最低 {money(Math.min(...profits))}</td><td><Badge>{statusOf(item)}</Badge></td><td><RowActions onEdit={() => onEdit(item)} onDelete={() => onDelete(item)} /></td></tr>; })}
...
function ProductForm({ editing, onSubmit, onClose }) {
  ...
  return <Modal title={editing ? '修改商品档案' : '新增商品档案'} onClose={onClose}><form onSubmit={onSubmit}><div className="form-grid"><Field label="商品类别"><select name="category" defaultValue={productCategoryOf(editing)}>{productCategories.map((category) => <option key={category}>{category}</option>)}</select></Field><Field label="商品名称"><input name="productName" defaultValue={editing?.productName} required /></Field></div><div className="spec-editor"><div className="spec-editor-head"><b>商品规格与成本价</b><button type="button" onClick={addSpec}>＋ 添加规格</button></div>{specRows.map((spec, index) => <div className="spec-edit-row" key={spec.id}><span>{index + 1}</span><input name="specName" value={spec.name} onChange={(event) => updateSpec(spec.id, 'name', event.target.value)} placeholder="例如：黑色-S" required /><input name="specCost" type="number" min="0" step="0.01" value={spec.cost} onChange={(event) => updateSpec(spec.id, 'cost', event.target.value)} placeholder="成本价" required /><button type="button" className="danger" disabled={specRows.length === 1} onClick={() => removeSpec(spec.id)}>删除</button></div>)}</div><FormActions onClose={onClose} /></form></Modal>;
}
